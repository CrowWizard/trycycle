...
DEFAULT_DB_PATH = Path.home() / ".local" / "share" / "kilo" / "kilo.db"
KILO_DATA_DIR_ENV = "KILO_DATA_DIR"
...
def _resolve_db_path(search_root: Path | None) -> Path:
    if search_root is not None:
        ...
        if db_path.exists():
            return db_path
        raise TranscriptError(f"Kilo database not found at {db_path}")

    data_dir = os.environ.get(KILO_DATA_DIR_ENV)
    if data_dir:
        db_path = Path(data_dir) / "kilo.db"
        if db_path.exists():
            return db_path
    ...
    raise TranscriptError(
        f"Kilo database not found at {DEFAULT_DB_PATH}. "
        f"Set {KILO_DATA_DIR_ENV} or pass --search-root."
    )
...
def find_current_transcript(search_root: Path | None = None) -> Path | None:
    ...
    if row:
        return Path(f"/kilo-session/{row['id']}")
    return None
...
def find_matching_transcripts(
    *,
    ...
    while True:
        ...
        try:
            rows = conn.execute(
                ...
                WHERE s.time_archived IS NULL AND (
                ...
                ORDER BY s.time_updated DESC, s.time_created DESC, p.time_created DESC
                LIMIT 1
                """,
                (f"%{canary}%", f"%{canary}%"),
            ).fetchall()
        ...
        if rows:
            return [Path(f"/kilo-session/{rows[0]['session_id']}")]
    ...
    raise TranscriptError(
        f"No Kilo session contained canary {canary!r} within {timeout_ms}ms."
    )
...
def extract_transcript(path: Path) -> list[TranscriptTurn]:
    ...
    db_path = (
        _last_resolved_db_path
        if _last_resolved_db_path is not None
        else _resolve_db_path(None)
    )
    conn = _connect(db_path)
    try:
